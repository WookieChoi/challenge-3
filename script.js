...
function promptForPass() {
  ...
  if (passLeng < 8 || passLeng > 128 || isNaN(passLeng)) {
    //error message
  ...
  //if yes
  if (confirm("Click to confirm including lowercase characters")) {
    //concat lowercase to final array
    passArray = passArray.concat(lowerArray);
  }

  //if yes
  if (confirm("Click to confirm including uppercase characters")) {
...
function generatePassword() {
  ...
  for(var i = 0; i < passLeng; i++) {
    ...
    password = password + passArray[scrambleIndex];
  }
  //return value
  return passReturn;
}
